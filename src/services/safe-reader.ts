...
function identityOf(stat: Awaited<ReturnType<fs.FileHandle["stat"]>>): OpenIdentity {
    return {
        dev: BigInt(stat.dev),
        ino: BigInt(stat.ino),
...
function sameIdentity(left: OpenIdentity, right: OpenIdentity): boolean {
    return left.dev === right.dev && left.ino === right.ino && left.size === right.size && left.mtimeNs === right.mtimeNs;
}
function checkBinary(buffer: Buffer): void {
...
export class SafeReader {
    public async readText(authorized: AuthorizedPath, request: ReadTextRequest): Promise<ReadTextResult> {
    ...
    public async readWholeText(authorized: AuthorizedPath, maxBytes: number, signal?: AbortSignal): Promise<{
        text: string;
        sha256: string;
        fileBytes: number;
    }> {
    ...
    public async readForMutation(authorized: AuthorizedPath, maxBytes: number, signal?: AbortSignal): Promise<WholeTextRead> {
        return this.#readWholeText(authorized, maxBytes, signal);
    }
    async #readWholeText(authorized: AuthorizedPath, maxBytes: number, signal?: AbortSignal): Promise<WholeTextRead> {
    ...
    public async sha256(authorized: AuthorizedPath, signal?: AbortSignal): Promise<string> {
        const flags = fsConstants.O_RDONLY | (fsConstants.O_NOFOLLOW ?? 0);
